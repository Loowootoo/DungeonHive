...
import (
	...
	"DungeonHive/internal/pathfinding"
)
...
func carveCorridor(m *[GridSize][GridSize]int, mapW, mapH int, start, end Point, r1, r2 Room) {
	...
	path := pathfinding.Search(start, end, costFn)
	if path == nil {
		return
	}
	for _, p := range path {
...
//   - 走既有通道        → 代價 1
//   - 挖新牆壁          → 代價 2
func makeCostFn(m *[GridSize][GridSize]int, mapW, mapH int, r1, r2 Room) pathfinding.CostFunc {
